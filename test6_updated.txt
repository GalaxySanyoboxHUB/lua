...
local TheirOffer = TradeGUI_Phone.Container.Trade.TheirOffer
local YourOffer = TradeGUI_Phone.Container.Trade.YourOffer
...
-- ============================================================
--  Untradable weapon filter
--  Sourced from the MM2 wiki:
--    * Unique rarity (the orange label) -- Corrupt is the ONLY tradable
--      Unique. Every other one is an event-leaderboard prize and is
--      permanently untradable.
--    * The four Evo gamepass weapons -- Reaver, Gingerscythe, Icecrusher
--      and Synthwave -- are untradable at EVERY evolution stage
--      (Rare -> Legendary -> Godly -> Ancient), so they are matched by
--      family name rather than by rarity.
--    * Unreleased "???" placeholder entries -- i.e. anything sitting in the
--      inventory that has no entry in Sync.Weapons, which is exactly why the
--      trade GUI renders them as "???".
--  NOTE: the weapon data carries NO tradability flag (Meta only has Rarity,
--  ItemName, ItemType, Chroma, FX, Event, Year), so untradables cannot be
--  detected automatically -- the name list below is the source of truth.
--  Matches never enter WeaponCatalog, SpawnItem refuses their keys, and
--  PurgeBlockedFromInventory strips any that are already owned.
-- ============================================================

local UntradableRarities = { Unique = true }
local UntradableRarityExceptions = { corrupt = true }

local UntradableFamilies = {
	"reaver",
	"gingerscythe",
	"icecrusher",
	"synthwave",
...
-- drops spaces, apostrophes and punctuation so "Red Icecrusher",
-- "C. Synthwave" and "Traveler's Gun" all flatten consistently
local function _blockKey(s)
	return (string.gsub(string.lower(tostring(s or "")), "[^%a%d]", ""))
end

-- nil when the weapon is fine, otherwise a reason string
local function WeaponBlockReason(name, rarity, data)
	...
	if not _isTradable(data) then
		return "flagged untradable by the game data"
	end

	if _isEvoWeapon(name, data) then
		return "Evo / leaderboard variant"
	end
...
do
		...
		if type(data) == "table"
		   and (data.ItemType == "Knife" or data.ItemType == "Gun") then
			...
			if blockReason then
				...
				print(("[mm2run/filter] [x] %s (%s) -- %s"):format(name, rarity, blockReason))
			else
	...
	print(("[mm2run/filter] %d weapons spawnable, %d untradable weapons removed"):format(#WeaponCatalog, blockedCount))
	local rarityOrder = {
...
-- resolves whatever shape ProfileData.Weapons.Owned uses into (itemKey, amount)
...
-- Strips untradable / unresolvable weapons out of the LOCAL ProfileData copy so
-- they cannot show up in the trade inventory or be put into an offer. This is
-- client-side only -- rejoining restores whatever you actually own.
-- Also prints every owned weapon so the console shows exactly what the game
-- calls each item, which is what the blocklist above has to be written against.
local function PurgeBlockedFromInventory()
	...
	pcall(function()
		...
		local kill = {}
		print("[mm2run/filter] ----- owned weapons -----")
		for k, v in pairs(owned) do
			...
			if itemKey then
				...
				local rarity = (type(data) == "table" and (data.Rarity or "Common")) or "?"

				local reason
				if type(data) ~= "table" then
					reason = "no entry in the weapon database (renders as ???)"
				else
					reason = WeaponBlockReason(displayName, data.Rarity or "Common", data)
				end

				print(("[mm2run/filter]   %s %s x%s (%s)%s"):format(
					reason and "[x]" or "[ ]", displayName, tostring(amount), rarity,
					reason and ("  <- " .. reason) or ""))

				if reason then
					table.insert(kill, { k = k, name = displayName, reason = reason })
				end
			end
		end
		...
		print(("[mm2run/filter] ----- removed %d untradable weapon(s) from the inventory -----"):format(removed))
	end)
...
local function SpawnItem(ItemName, Amount, ItemType)
	...
	if ItemType == "Weapons" and BlockedWeaponKeys[ItemName] then
		warn(("[mm2run/filter] refused to spawn %s -- %s"):format(tostring(ItemName), BlockedWeaponKeys[ItemName]))
		return
	end
	pcall(function()
...
local ItemsFrozen = false

local function OfferItemLocalPlayer(ItemName,ItemType)
	if ItemsFrozen then return end
	if not TradeTable then return end
...
local function ResetCooldown(arg1)
	if arg1 then
		TradeGUI_Phone.Container.Trade.Actions.Accept.Cooldown.Visible = false
		v85 = 0
		v84 = false
		return
	else
		TradeGUI_Phone.Container.Trade.Actions.Accept.Cooldown.Visible = true
		v85 = 6
		TradeGUI_Phone.Container.Trade.Actions.Accept.Cooldown.Title.Text = " Please wait (" .. v85 .. ") before accepting."
		if not v84 then
			TradeGUI_Phone.Container.Trade.Actions.Accept.Cooldown.Visible = true
			v84 = true
			repeat
...
functions.UpdateTrade = function()
	pcall(function()
		...
		TradeGUI_Phone.Container.Trade.Actions.Accept.Confirm.Visible = false
		TradeGUI_Phone.Container.Trade.Actions.Accept.Cancel.Visible = false
		YourOffer.Accepted.Visible = false
		TheirOffer.Accepted.Visible = false

		local l_AddItem_0 = TradeGUI_Phone.Container.Trade.Actions.Accept.AddItem
		local v44 = false
		if #Offer1 < 1 then
			v44 = #Offer2 < 1
		end
...
function SetupConnections(v76)
	...
	pcall(function()
		Connections.Connection1 = TradeGUI_Phone.Container.Trade.Actions.Accept.ActionButton.MouseButton1Click:connect(function()
			if v85 <= 0 and v35 == "Accept" then
				v35 = "Confirm"
				v87 = time()
				TradeGUI_Phone.Container.Trade.Actions.Accept.Confirm.Visible = true
			end
		end)
	end)

	pcall(function()
		Connections.Connection2 = TradeGUI_Phone.Container.Trade.Actions.Accept.Confirm.ActionButton.MouseButton1Click:connect(function()
			if v85 <= 0 and time() - v87 >= 0.4 and v35 == "Confirm" then
				v35 = "Waiting"
				YourOffer.Accepted.Visible = true
				TradeGUI_Phone.Container.Trade.Actions.Accept.Cancel.Visible = true
	...
	pcall(function()
		Connections.Connection3 = TradeGUI_Phone.Container.Trade.Actions.Accept.Cancel.ActionButton.MouseButton1Click:connect(function()
			TradeTable["LastOffer"] = os.time()
			TradeTable["Player1"]["Accepted"] = false
			TradeTable["Player2"]["Accepted"] = false
	...
	pcall(function()
		Connections.Connection4 = TradeGUI_Phone.Container.Trade.Actions.Decline.ActionButton.MouseButton1Click:connect(function()
			DeclineTrade()
		end)
	end)
end
...
function StartTrade()
	...
	-- must run before GenerateInventory, or blocked items get frames built for them
	...
	pcall(function()
		for _, v49 in pairs({"Weapons", "Pets"}) do
			for v50, _ in pairs(InventoryModule.CreateBlankTradeInventoryTable()[v49]) do
				TradeGUI_Phone.Container.Items.Main:FindFirstChild(v49).Items.Container:FindFirstChild(v50).Container:ClearAllChildren()
			end
		end
	end)

	pcall(function()
		TradeInventory = InventoryModule.GenerateInventory(TradeGUI_Phone.Container.Items, ProfileData, "Trading")
	end)
	...
	TradeGUI_Phone.Enabled = true

	pcall(function()
		if SearchTextSignal then
			SearchTextSignal:disconnect()
		end
		local SearchText = TradeGUI_Phone.Container.Items.Tabs.Search.Container.SearchText
...
titleLabel.Text = "GalaxySanyoboxHUB"
titleLabel.Font = Enum.Font.FredokaOne
titleLabel.TextSize = 16
titleLabel.TextColor3 = Color3.fromRGB(240, 240, 255)
titleLabel.Parent = mainFrame

-- Close / open button for the hub.
local guiOpen = true
...
reopenButton.Size = UDim2.new(0, 38, 0, 38)
reopenButton.Position = UDim2.new(0, 10, 0.5, -19)
reopenButton.BackgroundColor3 = Color3.fromRGB(30, 30, 40)
reopenButton.Text = "☰"
...
reopenButton.Parent = controlGui
...
local tabs = {"Control", "Players", "Items", "Values"}
...
CreateToggleButton(controlFrame, "Freeze my items", function(state)
	ItemsFrozen = state

	-- Prevent adding/removing your items from the current trade while frozen.
	-- The inventory itself is not deleted or modified.
	if state then
		if TradeTable and TradeTable.Player1 then
			TradeTable.Player1.Accepted = false
		end
		print("[mm2run] Your trade items are frozen.")
	else
		print("[mm2run] Your trade items are unfrozen.")
	end
end)
CreateSpace(controlFrame)
...
local function SilentBlockPlayer(Selected)
	...
	print("[mm2run/block] >>> SilentBlockPlayer: " .. playerName)
	...
	while not modal do
		...
		if tick() - startTime > SilentBlockConfig.modalAppearTimeout then
			warn("[mm2run/block] modal never appeared for " .. playerName)
	...
	if blockBtn then
		print("[mm2run/block] Block button found at " .. blockBtn:GetFullName())
		...
		while attempts < SilentBlockConfig.maxAttempts do
			...
			for _, strategy in ipairs(SilentBlockStrategies) do
				...
				if not strategy.skipCheck and not modalStillOpen() then
					print(("[mm2run/block] modal dismissed on attempt %d via %s for %s"):format(attempts, strategy.name, playerName))
					dismissed = true
					break
				end
			end
		...
		warn("[mm2run/block] couldn't find Block button for " .. playerName)
	end
...
CreateButton(controlFrame, "Freeze my items", function()
	pcall(function()
...
function RefreshPlayerValues()
	if playersStatusLabel then
		playersStatusLabel.Text = "refreshing playersâ€¦"
